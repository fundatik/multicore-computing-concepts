...
int main(int argc, char *argv[])
{
    ...
    printf("NAS Parallel Benchmarks 4.0 OpenMP C++ version"
           " - MG Benchmark\n");
    printf("Developed by: Dalvan Griebler <dalvan.griebler@acad.pucrs.br> & Júnior Löff <loffjh@gmail.com>\n\n");
    printf("Running on Cheetah Cilk with %d workers\n\n", num_workers);
    fp = fopen("mg.input", "r");
    if (fp != NULL)
    {
    ...
    setup(&n1, &n2, &n3, lt);
    worker_tmp1 = (double **)calloc(num_workers, sizeof(double *));
    worker_tmp2 = (double **)calloc(num_workers, sizeof(double *));
    worker_tmp3 = (double **)calloc(num_workers, sizeof(double *));
    for (int w_number = 0; w_number < num_workers; w_number++)
    {
        worker_tmp1[w_number] = (double *)calloc(M, sizeof(double));
        worker_tmp2[w_number] = (double *)calloc(M, sizeof(double));
        worker_tmp3[w_number] = (double *)calloc(M, sizeof(double));
    }
...
static void psinv(double ***r, double ***u, int n1, int n2, int n3, double c[4], int k)
{
    ...
    cilk_for(int i3 = 1; i3 < n3 - 1; i3++)
    {
        int w = __cilkrts_get_worker_number();
        double *r1 = worker_tmp1[w];
        double *r2 = worker_tmp2[w];
...
static void resid(double ***u, double ***v, double ***r, int n1, int n2, int n3, double a[4], int k)
{
    ...
    {
        int w = __cilkrts_get_worker_number();
        double *u1 = worker_tmp1[w];
        double *u2 = worker_tmp2[w];
        for (int i2 = 1; i2 < n2 - 1; i2++)
        {
            for (int i1 = 0; i1 < n1; i1++)
            {
...
static void rprj3(double ***r, int m1k, int m2k, int m3k, double ***s, int m1j, int m2j, int m3j, int k)
{
    ...
    cilk_for(int j3 = 1; j3 < m3j - 1; j3++)
    {
        ...
        double x2, y2;
        int w = __cilkrts_get_worker_number();
        double *x1 = worker_tmp1[w];
        double *y1 = worker_tmp2[w];
        ...
        for (j2 = 1; j2 < m2j - 1; j2++)
        {
            ...
            for (j1 = 1; j1 < m1j - 1; j1++)
            {
                ...
                s[j3][j2][j1] =
                    0.5 * r[i3 + 1][i2 + 1][i1 + 1] + 0.25 * (r[i3 + 1][i2 + 1][i1] + r[i3 + 1][i2 + 1][i1 + 2] + x2) + 0.125 * (x1[i1] + x1[i1 + 2] + y2) + 0.0625 * (y1[i1] + y1[i1 + 2]);
            }
        }
    }
...
static void interp(double ***z, int mm1, int mm2, int mm3, double ***u, int n1, int n2, int n3, int k)
{
    ...
    */
    ...
    if (n1 != 3 && n2 != 3 && n3 != 3)
    {
        cilk_for(int i3 = 0; i3 < mm3 - 1; i3++)
        {
            int w = __cilkrts_get_worker_number();

            double *z1 = worker_tmp1[w];
            double *z2 = worker_tmp2[w];
            double *z3 = worker_tmp3[w];
            ...
            for (int i2 = 1; i2 <= mm2 - 1; i2++)
            {
                ...
                for (int i1 = 1; i1 <= mm1 - 1; i1++)
                {
                    u[2 * i3 - d3 - 1][2 * i2 - t2 - 1][2 * i1 - t1 - 1] =
                        u[2 * i3 - d3 - 1][2 * i2 - t2 - 1][2 * i1 - t1 - 1] + 0.25 * (z[i3 - 1][i2][i1] + z[i3 - 1][i2 - 1][i1] + z[i3 - 1][i2][i1 - 1] + z[i3 - 1][i2 - 1][i1 - 1]);
                }
            }
        }

        cilk_for(int i3 = 1; i3 < mm3; i3++)
        {
            for (int i2 = d2; i2 <= mm2 - 1; i2++)
            {
                for (int i1 = d1; i1 <= mm1 - 1; i1++)
                ...
                for (int i1 = 1; i1 <= mm1 - 1; i1++)
                {
                    u[2 * i3 - t3 - 1][2 * i2 - d2 - 1][2 * i1 - t1 - 1] =
                        u[2 * i3 - t3 - 1][2 * i2 - d2 - 1][2 * i1 - t1 - 1] + 0.25 * (z[i3][i2 - 1][i1] + z[i3][i2 - 1][i1 - 1] + z[i3 - 1][i2 - 1][i1] + z[i3 - 1][i2 - 1][i1 - 1]);
                }
            }
            for (int i2 = 1; i2 <= mm2 - 1; i2++)
            {
                for (int i1 = d1; i1 <= mm1 - 1; i1++)
                {
                    u[2 * i3 - t3 - 1][2 * i2 - t2 - 1][2 * i1 - d1 - 1] =
                        u[2 * i3 - t3 - 1][2 * i2 - t2 - 1][2 * i1 - d1 - 1] + 0.25 * (z[i3][i2][i1 - 1] + z[i3][i2 - 1][i1 - 1] + z[i3 - 1][i2][i1 - 1] + z[i3 - 1][i2 - 1][i1 - 1]);
                }
                for (int i1 = 1; i1 <= mm1 - 1; i1++)
                {
                    u[2 * i3 - t3 - 1][2 * i2 - t2 - 1][2 * i1 - t1 - 1] =
                        u[2 * i3 - t3 - 1][2 * i2 - t2 - 1][2 * i1 - t1 - 1] + 0.125 * (z[i3][i2][i1] + z[i3][i2 - 1][i1] + z[i3][i2][i1 - 1] + z[i3][i2 - 1][i1 - 1] + z[i3 - 1][i2][i1] + z[i3 - 1][i2 - 1][i1] + z[i3 - 1][i2][i1 - 1] + z[i3 - 1][i2 - 1][i1 - 1]);
                }
            }
        }
    }
...
static void norm2u3(double ***r, int n1, int n2, int n3, double *rnm2, double *rnmu, int nx, int ny, int nz)
{
    ...
    double *p_s_ff = new double[num_workers];
    double *p_a_ff = new double[num_workers];
    ...
    cilk_for(uint64_t i3 = 1; i3 < n3 - 1; i3++)
    {
        ...
        for (int i2 = 1; i2 < n2 - 1; i2++)
        {
            for (int i1 = 1; i1 < n1 - 1; i1++)
            {
                p_s_ff[id] += r[i3][i2][i1] * r[i3][i2][i1];
                double tmp = fabs(r[i3][i2][i1]);
                if (tmp > p_a_ff[id])
                    p_a_ff[id] = tmp;
            }
        }
