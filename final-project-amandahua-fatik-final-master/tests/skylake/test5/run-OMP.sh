...
do
    for perf_group in ${lst_perf_groups[@]}; do
		...
		likwid-perfctr -C ${lst_core_ids[$i]} -g ${perf_group} -m bin/$app &> ../tests/$machine/$test/${out_file}
		...
		c=${lst_nr_cores2[$i]}
      		...
      		echo Trial $k of running $app with $imp and $c cores >&2
        	export OMP_NUM_THREADS=$c; export OMP_PROC_BIND=true;
		likwid-perfctr -C ${lst_core_ids2[$i]} -g ${perf_group} -m bin/$app &> ../tests/$machine/$test/${out_file}
		ts=$(grep "Time in seconds" ../tests/$machine/$test/${out_file})
        	echo $c, $ts
    	done
    done
